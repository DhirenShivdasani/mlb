...
def push_to_github():
    try:
        ...
        download_from_s3(BUCKET_NAME, 'merged_wnba.csv', 'merged_wnba.csv')

        # Ensure file system registers the changes
        time.sleep(2)

        # Force update file timestamp
        os.utime('merged_wnba.csv', None)
...
prizepicks_data = pd.read_csv('test3.csv')
...
prizepicks_data = prizepicks_data[(prizepicks_data['Prop'] == 'Points') | 
                                  (prizepicks_data['Prop'] == 'Assists') | 
                                  (prizepicks_data['Prop'] == 'Rebounds')|
                                  (prizepicks_data['Prop'] == '3-PT Made')]
prop_types = {
    'Points': 'Points',
    'Assists': 'Assists',
    'Rebounds': 'Rebounds',
...
if os.path.exists('merged_wnba.csv'):
    os.remove('merged_wnba.csv')

# Save the updated data to the file
r.to_csv('merged_wnba.csv', index=False)

upload_to_aws('merged_wnba.csv', BUCKET_NAME, 'merged_wnba.csv')

s3_file = 'merged_wnba.csv'
local_file = 'merged_wnba.csv'
